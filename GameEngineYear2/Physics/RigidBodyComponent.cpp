...
void GamePhysics::RigidBodyComponent::clampVelocity(float maxSpeed)
{
    float speedSquared = m_velocity.x * m_velocity.x + m_velocity.y * m_velocity.y;
...
void GamePhysics::RigidBodyComponent::fixedUpdate(float fixedDeltaTime)
{
    ...
    if (std::isnan(getVelocity().x)) {
        setVelocity({ 0, 0 });
        newPosition = { static_cast<float>(rand() % 800 - 275), static_cast<float>(rand() % 800 - 275) };
    }
    ...
    clampVelocity(300.0f); // Adjust this value as needed
}
